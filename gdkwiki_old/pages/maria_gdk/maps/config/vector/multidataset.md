...
Note that all entries in this file are treated as hints and may be overridden by the map renderer. Also, all “limits” refer to selected scalebase (nominal or actual) multiplied by nominalscalefactor. For “nominal”, these do not usually match actual map scale.
`</WRAP>`

`<WRAP center round info 100%>`
Note that all limits can be given as zoomlevels since version 2.0. Use postfix "L" to parse limits as zoomlevel instead of mapscale (f.ex. minvisible="15L" maxvisible="6L"). Mapscales will be converted to corresponding zoomlevels.
Thinningtab and scalebase is obsolete since version 2.0.
`</WRAP>`
...
**Note:** groupsettings under globalsettings act as a default template for the groupsettings listed in the groupsettingtemplate section of the xml.
...
**Note:** clipborder is only used when drawing points.
